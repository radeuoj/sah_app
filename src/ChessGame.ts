...
export function useChessGame(): ChessGame {
    ...
    function requestMove(piece: ChessPiece, pos: Vector2): boolean {
        const nextMove = suggestMoves(piece).find((m) => m.to.x == pos.x && m.to.y == pos.y);
        if (nextMove)
            move(nextMove, pieces, setPieces, moves, setMoves);
    ...
    function suggestMoves(piece: ChessPiece): ChessMove[] {
        return suggestMovesUnsafe(piece, pieces, moves);
    }
